$(document).ready(function(){
    ...
    $(document).on('change','#facture',function(event) {
        event.preventDefault();

        var facture = $(this).children('option:selected').val();

        var url = Routing.generate('bon_commande_facture', { facture : facture });
	...
	$(document).on('click', '.btn-add-row', function(event) {
        ...
        var type_designation_options = $('.type_designation').html();

        var a = '<td><div class="form-group"><div class="col-sm-12"><select class="form-control type_designation" name="type_designation[]">'+ type_designation_options +'</select></div></div></td>';
    ...
    $(document).on('click', '#btn-save', function(event) {
        ...
        $('.type_designation').each(function(){
            if($(this).val() == 1)
            {
                $(".f_ps_qte").each(function(){
                    var val_qte = $(this).val()
                    if(parseInt(val_qte) < 0)
                    {
                        negatif = true
                        enregistre = false
                        return false
        ...
        if(enregistre)
    	{
            ...
            var data = {
                facture : $('#facture').val(),
                modele : $('#facture').children('option:selected').data('modele'),
                client : $('#client').val(),
            ...
            var url = Routing.generate('bon_commande_save');
            ...
            swal({
                ...
                function () {
                    ...
                    $.ajax({
                        ...
                        data: data,
                        success: function(res) {
                            show_success('Succès', 'Bon de commande enregistré');
                        }
                    });
        ...
        else
        {
            if(negatif)
            {
                swal({
                    ...
                    text: "Vérifier tous les quantités",
